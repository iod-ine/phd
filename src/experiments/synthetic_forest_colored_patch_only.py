...
from src.datasets import SyntheticForestRGBPatch
...
class SyntheticForestColoredDataModule(L.LightningDataModule):
    ...
    def prepare_data(self):
        ...
        SyntheticForestRGBPatch(**self.dataset_params)

    def setup(self, stage: str):
        """Prepare the data for training (split, transform, etc.) on all devices."""
        if stage == "fit":
            self.train = SyntheticForestRGBPatch(
                split="train",
                **self.dataset_params,
                transform=self.transform,
            )
            self.val = SyntheticForestRGBPatch(
                split="val",
                **self.dataset_params,
